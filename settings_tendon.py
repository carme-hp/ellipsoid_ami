...
#   for j in range(ny):
#     for i in range(nx):
#       initial_displacements[k*nx*ny + j*nx + i] = [0.0, 0.0,  -a + a*k/4 ]

# for k in range(4):
#   for j in range(ny):
#     for i in range(nx):
#       initial_displacements[4*nx*ny + k*nx*ny + j*nx + i] = [0.0, 0.0,  a*k/4 ]
...
config = {
  ...
  "PreciceAdapter": {        # precice adapter for bottom tendon
    ...
    "DynamicHyperelasticitySolver": {
      ...
      # mesh
      "meshName":                   "3Dmesh_tendon",           # mesh with quadratic Lagrange ansatz functions
